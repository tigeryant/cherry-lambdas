...
def lambda_handler(event, context):
    ...
    USERNAME = parameter_values['SSH_USER']
    PASSWORD = parameter_values['SSH_PASSWORD']

    response_object = {}
    response_object['headers'] = {}
    response_object['headers']['Content-type'] = 'application/json'

    if 'txid' in event['queryStringParameters']:
        if event['queryStringParameters']['txid'] != "":
            txid = event['queryStringParameters']['txid']
        else:
            response_object['statusCode'] = 400
            response_object['body'] = json.dumps({"error": "txid parameter value was missing from request"})
            return response_object
    else:
        response_object['statusCode'] = 422
        response_object['body'] = json.dumps({"error": "txid parameter key was missing from request"})
        return response_object
